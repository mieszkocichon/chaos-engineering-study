#!/usr/bin/env python3
"""
analyze_results.py — Analiza wyników macierzy eksperymentalnej.

Odpowiada na zarzuty recenzenta:
  2.1  Localhost ≠ distributed  →  sieć Docker bridge + tc netem (weryfikacja strukturalna)
  2.2  Gossip N=3-4 bezcelowy  →  N=20, 3-poziomowa topologia (z wyników)
  Fault injection              →  stateHistory CB vs throttleLog backpressure
  Novelty backpressure         →  t_throttle < t_CB_open (pre-empcja przed reaktywnym CB)
"""
...
# ── Analiza Circuit Breaker (strategie reaktywne) ─────────────────────────────
...
# ── Analiza Backpressure (strategia pre-emptywna) ─────────────────────────────
...
# ── Analiza nowości: pre-empcja backpressure vs reaktywność CB ────────────────
    ...
    for fault in FAULT_ORDER:
        ...
        if 'backpressure' in by_fault[fault]:
            for entry in by_fault[fault]['backpressure']:
                ...
                # Sample throttle reasons to show gossip basis
                if bp_events:
                    sample = bp_events[:3]
...
# ── Analiza SPOF: porównanie strategii przy permanentnej awarii węzła ──────────
...
def print_spof_analysis(results):
    ...
    # Kluczowe porównanie: mas-agent vs central (szybkość detekcji przez gossip)
...
# ── Tabela porównawcza success rate per fault ─────────────────────────────────
...
# ── Odpowiedzi na zarzuty recenzenta ─────────────────────────────────────────
...
def mann_whitney_p(a, b):
    ...
    if _SCIPY:
        try:
            ...
            pass
    # Aproksymacja normalna (wystarczająca dla n>=5)
    n1, n2 = len(a), len(b)
    if n1 == 0 or n2 == 0:
        return float('nan')
...
def print_conclusions(results):
    ...
    # Zbierz surowe dane per (strategy, fault)
    sr_data  = defaultdict(lambda: defaultdict(list))  # [strategy][fault] → [sr%]
    lat_data = defaultdict(lambda: defaultdict(list))  # [strategy][fault] → [avgMs]
    seed_set = defaultdict(set)                         # [strategy] → zbiór seed
    ...
    # ── 2. Porównanie SR: aktywne strategie vs. baseline ─────────────────────
    ...
    # ── 3. Latencja: MAS-Agent vs. Static i Central ───────────────────────────
    ...
    # ── 4. Kluczowe wnioski narracyjne ────────────────────────────────────────
    print('── 4. Wnioski narracyjne (gotowe do sekcji Discussion) ──────────────────────────')

    # Sprawdź czy MAS osiąga 100% SR wszędzie
    mas_perfect = all(
...
""")
    ...
    # ── 5. Wpływ wprowadzonych zmian na wyniki ─────────────────────────
    print('── 5. Wpływ aktualizacji na wyniki eksperymentów ─────────────────────────')
    print("""
  Poprawka 1 — CentralController 
  ...
  Poprawka 5 — Circular routing w topologii ring 
  ...
  Poprawka 6 — Profile awarii celowały w nieistniejący serwis 'svc-b' 
